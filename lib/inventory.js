...
// Fetch all values of a single sheet in the inventory spreadheet,
// and validate them against expected headers.
const fetchAndValidateInventorySheetValues = _.memoize(
...
// Return data from a given sheet as an array of “inventory row” objects.
...
export const fetchInventoryRows = async (
  ...
  const values = await fetchAndValidateInventorySheetValues(
    sheetName,
    expectedColumnHeaders
  );
  ...
  // e.g.: `dataValues` = [["1234", "TOY", "", "ABC", "Oui", ""], …]
  ...
  const inventoryRows = dataValues.map(
  ...
  return rejectEmptyValues(_.filter(inventoryRows, filter));
};
