...
const fetchDatabaseSheetValues = _.memoize(async (sheetName) => {
...
const fetchAndValidateDatabaseSheetValues = _.memoize(
  async (sheetName, expectedColumnHeaders) => {
    const values = await fetchDatabaseSheetValues(sheetName);
...
export const fetchDataRows = async (
  ...
  const values = await fetchAndValidateDatabaseSheetValues(
    sheetName,
    expectedColumnHeaders
  );
